...
public class GoogleSimilarityDistance {
	...
	public double ReadCache(String id) {
		String content = Utilities
				.readFileAsString("google-distance-cache.txt");
	...
	public double SimilarityFlex(String query_term1, String query_term2,
			String query_both_terms) {
		String wid = Utilities.Base64Encode(query_term1 + query_term2
				+ query_both_terms);
		...
		last_query = "http://www.google.com/search?q="
				+ Utilities.EncodeQuery(query_term1);
		...
		if (content.indexOf("did not match any documents.") > 0)
			...
			String temp = Utilities.SimpleRegexSingle(resultParser, content, 1)
					.replace(",", "");
			if (temp.equals(""))
				return 1;
			countw1 = Double.parseDouble(temp);
		}
		// //////////////

		// count term 2
		last_query = "http://www.google.com/search?q="
				+ Utilities.EncodeQuery(query_term2);
		...
		if (content.indexOf("did not match any documents.") > 0)
			...
			if (temp.equals(""))
				return 1;
			countw2 = Double.parseDouble(temp);
		}
		// /////////

		// count both
		last_query = "http://www.google.com/search?q="
				+ Utilities.EncodeQuery(query_both_terms);
		content = client.GetMethod(last_query);
		double countw12;
		if (content.indexOf("did not match any documents.") > 0)
			countw12 = 1;
	...
	public double Similarity(String w1, String w2, String context,
			String middle, String ex) {
		try {
			/*
			 * MeasurePair cache = MeasurePair.IsScored(w1, w2, pairs); if
			 * (cache != null) return cache.score;
			 */
			...
			if (client == null)
				client = new WebClientX();
			last_query = "http://www.google.com/search?q="
					+ Utilities.EncodeQuery((w1 + " " + ex + " " + context)
							.replace("  ", " "));
			String content = client.GetMethod(last_query);
			if (content.indexOf("did not match any documents.") > 0)
				return 1;
			...
			last_query = "http://www.google.com/search?q="
					+ Utilities.EncodeQuery((w2 + " " + ex + " " + context)
							.replace("  ", " "));
			content = client.GetMethod(last_query);
			if (content.indexOf("did not match any documents.") > 0)
				return 1;
			...
			// e.printStackTrace();
			// }
			temp = Utilities.SimpleRegexSingle(resultParser, content, 1)
					.replace(",", "");
			if (temp.equals(""))
				return 1;
			double countw2 = Double.parseDouble(temp);
			last_query = "http://www.google.com/search?q="
					+ Utilities.EncodeQuery((context + "" + w1 + " " + middle
							+ " " + w2 + " " + ex).replace("  ", " ").trim());
			content = client.GetMethod(last_query);
			if (content.indexOf("did not match any documents.") > 0)
				return 1;
